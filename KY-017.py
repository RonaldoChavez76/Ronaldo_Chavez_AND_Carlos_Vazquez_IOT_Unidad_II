...
MQTT_TOPIC = "utng/ky017"
MQTT_CLIENT_ID = "ESP32_KY017"
MQTT_PORT = 1883

# Configuración del sensor KY-017 en el pin 5
interruptor_mercurio = Pin(5, Pin.IN)
...
# Bucle principal: Detectar cambios de estado y enviar actualización por MQTT
estado_anterior = None

while True:
    estado_actual = interruptor_mercurio.value()
    
    if estado_actual != estado_anterior:  # Solo enviar datos si hay cambio
        if estado_actual == 1:
            mensaje = "Interruptor de mercurio ACTIVADO (Inclinado)"
        else:
            mensaje = "Interruptor de mercurio DESACTIVADO (Posición normal)"
